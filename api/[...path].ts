...
app.put('/api/admin/content', authMiddleware, async (req, res) => {
  try {
    ...
    const updated = await writeDatabaseAsync({ content });
...
app.put('/api/admin/theme', authMiddleware, async (req, res) => {
  try {
    ...
    const updated = await writeDatabaseAsync({ theme });
...
app.put('/api/admin/sections', authMiddleware, async (req, res) => {
  try {
    ...
    const updated = await writeDatabaseAsync({ sections });
...
app.put('/api/admin/seo', authMiddleware, async (req, res) => {
  try {
    const seo = req.body;
    const updated = await writeDatabaseAsync({ seo });
...
app.post('/api/admin/media', authMiddleware, async (req, res) => {
  try {
    ...
    // Vercel filesystem ماشي دائم، لذلك نخزن الصورة كـ dataUrl في Supabase settings
    ...
    await writeDatabaseAsync({ media: updatedMedia });
...
app.delete('/api/admin/media/:id', authMiddleware, async (req, res) => {
  try {
    ...
    await writeDatabaseAsync({ media: filtered });
